  ...
  return (
    <DropdownMenu>
          ...
          <DropdownMenuContent className="w-56">
            <DropdownMenuLabel className="cursor-pointer" onClick={(e) => handleClick(e, { name: '', value: '' })}>Types(click to deselect)</DropdownMenuLabel>
            <DropdownMenuSeparator />
            {types.map((item:types) => (
              <DropdownMenuItem key={item.id} onClick={(e) => handleClick(e, item)}>{item.name}</DropdownMenuItem>
            ))}
          </DropdownMenuContent>
        </DropdownMenu>
  )
