    ...
    <div>
      <div className="flex flex-col justify-start items-center w-full mt-10">
        <div className="flex flex-row justify-center items-center w-2/3 bg-white p-3 rounded-t-lg">
          <Image src={`/images/townHalls/th${player.townHallLevel}.png`} alt="player league" width={100} height={100 } style={{ backgroundColor: 'transparent', width: 'auto', height: 'auto' }} unoptimized/>
        ...
        <div className='w-full lg:w-2/3 bg-white p-3 rounded-b-lg'>
          <Accordion type='single' collapsible className='w-full'>
            <AccordionItem value='item-1'>
              ...
              <AccordionContent>
                <div className='text-neutral-900 font-extrabold'>
                  <div className='flex flex-col justify-center items-center w-full m-2' key={basicInfoData[0].name}>
                    <Image src={basicInfoData[0].value} alt="player league" width={50} height={50} style={{ width: 'auto', height: 'auto' }} className=''/>
            ...
            <AccordionItem value='item-2'>
              ...
              <AccordionContent>
                <div className='text-neutral-900 font-extrabold'>
                  <div className='flex flex-row justify-center items-center w-full m-2' key={clanData[0].name}>
                    <Image src={clanData[0].value} alt="player league" width={50} height={50} style={{ width: 'auto', height: 'auto' }} className=''/>
                  </div>
                  {
            ...
            <AccordionItem value='item-3'>
              ...
              <AccordionContent>
                <div className='text-neutral-900 font-extrabold'>
                  {player.troops.filter((troop: any) => troop.name.includes('Super') === false).map((troop: any) => (
                    <div className='flex flex-row justify-between items-center w-full hover:bg-gray-100 p-2 rounded-lg cursor-default' key={troop.name}>
                      <span>{troop.name}</span>
                      <span><span className={troop.level !== troop.maxLevel ? 'text-gray-400' : ''}>{troop.level}</span>{' '} / {troop.maxLevel}</span>
                    </div>
                  ))}
                </div>
              </AccordionContent>
                  ...
                  {player.achievements.map((achievement: any) => (
                    <HoverCard key={achievement.name}>
                      <HoverCardTrigger className='flex flex-row justify-between items-center w-full hover:bg-gray-100 p-2 rounded-lg cursor-default'>
                        <span>{achievement.name} | {achievement.value}  / <span className={achievement.value < achievement.target ? 'text-gray-400' : ''}>{achievement.target}</span></span>
                        <span><span className={achievement.stars !== 3 ? 'text-gray-400' : ''}>{achievement.stars}</span>{' '} / 3</span>
                    </HoverCardTrigger>
